...
class SchedulesController extends Controller
{
    ...
    public function viewSeatInfo(Request $request)
    {
        // pass important params with flow of it from page to page
        $schedules_id = $request->schedules_id;
        $data = Schedules::where('schedules_id', $schedules_id)->first();
        $buses_id=$data->buses->buses_id;
        $seat = json_decode($data->seat_info, true);
        $front = json_decode($data->front_info, true);
        return view('backend.schedule.view_seat_layout', ['seat' => $seat, 'buses_id' => $buses_id, 'schedules_id' => $schedules_id, 'front' => $front,]);
